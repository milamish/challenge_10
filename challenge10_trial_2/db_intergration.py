...
def users():
    ...
    try:
        ...
        with connection.cursor() as cursor:
            sql="INSERT INTO `users`(`Name`,`Email_adress`,`Password`,`Username`,`User_type`) VALUES(%s,%s,%s,%s,%s)"
            try: 
                cursor.execute("SELECT * FROM `users` WHERE `Username`=%s;",Username)
                if cursor.fetchone() is not None:
                    return jsonify({"message":"Username taken"})
                else: 
                    cursor.execute(sql,(Name,Email_adress,Password,Username,User_type))
            except:
...
def comments():
    ...
    try:
        connection = pymysql.connect(host ='localhost',user='root',password='',db='flask_api')
        with connection.cursor() as cursor:
            sql_com= "INSERT INTO `user_comments`(`comment`,`User_ID`) VALUES(%s,%s)"
            try: 
                cursor.execute(sql_com,(comment,User_ID))
            except:
                return jsonify({"message":"oops!"})
        connection.commit()
    finally:
